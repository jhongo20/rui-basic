...
function createObservation(field, event, idoniedadId = null) {
    ...
    if (idoniedadId) {
        ...
        currentField = null;
        currentIdoniedadId = idoniedadId;
        ...
        .then(data => {
            ...
            showObservationDialog(null, null, idoniedadId);
...
function removeObservation(field, event, idoniedadId = null) {
    if (event) event.preventDefault();
...
function showObservationDialog(field, event, idoniedadId = null) {
    if (event) event.preventDefault();

    const intermediaryId = document.getElementById('intermediaryId').value;

    // Almacenar los valores para guardar
    document.getElementById('currentField').value = field || '';
    document.getElementById('currentIdoniedadId').value = idoniedadId || '';

    console.log("showObservationDialog called with field:", field, "idoniedadId:", idoniedadId);
                ...
                if (!response.ok) {
                    if (response.status === 404) {
                        ...
                        document.getElementById('observationLabel').textContent = 
                            `Observación sobre el registro de idoneidad (ID: ${idoniedadId})`;
        ...
        .then(response => {
            ...
            if (response.ok) {
                ...
                if (buttonsContainer) {
                    ...
                    console.error('Contenedor de botones no encontrado para currentField:', currentField);
                }
